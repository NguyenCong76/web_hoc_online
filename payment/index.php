...
<title>Thanh toán khóa học online.</title>
<style>
...
<body>	
	...
	<form  name="NLpayBank" action="#" method="post">	
		<table style="clear:both;width:500px;padding-left:46px;"> 
			...
			<tr>
			<td>Chọn khóa học thanh toán:</td>
			<td>
		   <?php
            $ketqua=mysqli_query($p->connect(),"SELECT * FROM `hocsinh_khoahoc` WHERE ID_HS=".$_SESSION['id_user']." AND Payment='no'");
			$i=@mysqli_num_rows ($ketqua);
			if($i>0)
			{
				echo '<select name="khoa" id="khoa">
				<option>Chọn khóa học </option>';
			
            while($row = @mysqli_fetch_array($ketqua))
			{ 
				$id_khoa = $row['ID_khoa'];
				echo'<option value="'.$id_khoa.'">'.$p->laycot($p->connect(),"SELECT Ten_khoa FROM khoa_hoc WHERE ID_khoa ='$id_khoa'").'</option>';
			}
			echo '</select>';
		}
		   ?>
	...
	{ 
		...
		$sql="UPDATE `hocsinh_khoahoc` SET `Payment`= 'yes' WHERE `hocsinh_khoahoc`.`ID_HS` = ".$_SESSION['id_user']." AND `hocsinh_khoahoc`.`ID_khoa` ='$id_k'";
